...
<div class="col-md-8 col-xs-12 featured-left">
											...
											<div class="products-inner">
												...
												{if isset($product.new) && $product.new == 1}
													<a class="new-box" href="{$product.link|escape:'html':'UTF-8'}">
														<span class="new-label">{l s='New' mod='posfeaturedproducts'}</span>
													</a>
												{/if}
												{if isset($product.on_sale) && $product.on_sale && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}
													<a class="sale-box" href="{$product.link|escape:'html':'UTF-8'}">
														<span class="sale-label">{l s='Sale!' mod='posfeaturedproducts'}</span>
													</a>
												{/if}
											<!-- 	<div class="quick-views">
													{if isset($quick_view) && $quick_view}
														<a class="quick-view" title="{l s='Quick view' mod='posfeaturedproducts'}" href="{$product.link|escape:'html':'UTF-8'}">
															<span>{l s='Quick view' mod='posfeaturedproducts'}</span>
														</a>
													{/if}
												</div>	 -->							
											</div>
														...
														<ul class="add-to-links">
															...
															<div class="cart">
																...
																{if isset($static_token)}
																	<a class="ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow"  title="{l s='Add to cart' mod='posfeaturedproducts'}" data-id-product="{$product.id_product|intval}">
																		<span>{l s='Add to cart' mod='posfeaturedproducts'}</span>
																		
																	</a>
																{else}
																<a class="ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, 'add=1&amp;id_product={$product.id_product|intval}', false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='posfeaturedproducts'}" data-id-product="{$product.id_product|intval}">
																	<span>{l s='Add to cart' mod='posfeaturedproducts'}</span>
																</a>
																   {/if}      
																{else}
															...
															{if isset($comparator_max_item) && $comparator_max_item}
															  <a class="add_to_compare" href="{$product.link|escape:'html':'UTF-8'}" data-id-product="{$product.id_product}" title="{l s='Add to Compare' mod='posfeaturedproducts'}">{l s='Compare' mod='posfeaturedproducts'}
	...
	<script>
		$(document).ready(function() {
			...
			featuredSlide.owlCarousel({
				...
				itemsDesktop : [1400,{$slider_options.items_md}],
				itemsDesktopSmall : [1024,{$slider_options.items_sm}],
