	  ...
	  beforeSend: function() {
		...
		success: function(json) {
			if (json['error']) {
						...
						$('#commentInput').before('<div class="warning">  <i class="icon-warning-sign icon-lg"></i> {l s='Warning: Please check required form bellow!'  mod='smartblog'}  </div>');
						
						if (json['error']['name']) {
							$('.inputName').after('<span class="error">{l s='Name is required'  mod='smartblog'}</span>');
						}
						if (json['error']['mail']) {
							$('.inputMail').after('<span class="error">{l s='E-mail address not valid !'  mod='smartblog'}</span>');
						}
						if (json['error']['comment']) {
							$('.inputContent').after('<span class="error">{l s='Comment must be between 25 and 1500 characters!'  mod='smartblog'}</span>');
						}
						if (json['error']['captcha']) {
							$('.smartblogcaptcha').after('<span class="error">{l s='Captcha is not valid'  mod='smartblog'}</span>');
						}
					}
