...
class NetReviewsModel extends ObjectModel
{
        ...
        foreach ($item_list as $item) {
            ...
            foreach ($product_list as $product) {
                ...
                $all_orders[$product['id_order']]['PRODUCTS'][] = array(
                    ...
                    'UPC_PRODUCT' =>  (isset($o_product->upc) && !empty($o_product->upc))?$o_product->upc:'',
                ...
                foreach ($all_orders as $order) {
                    ...
                    if ($avis_produit == 1 && $count_products > 0) {
                        for ($i = 0; $i < $count_products; $i++) {
                            ...
                            $num_state = $order['ORDER_STATE'];
                            $line[] = $num_state; //Order state ID
                            $line[] = $order_statut_indice[$num_state]; //Etat de la commande
    ...
    public static function getUrlsProduct($product_id, $lang_id)
    {
        ...
        if ($product_exist) {
            ...
            if ((version_compare(_PS_VERSION_, '1.5', '>='))) {
                ...
                $image_path = $link->getImageLink($o_product->link_rewrite, $id_cover_image['id_image'], $img_type);
            } elseif ((version_compare(_PS_VERSION_, '1.4', '>='))) {
                $image_path = $link->getImageLink($o_product->link_rewrite, $product_id.'-'.$id_cover_image['id_image'], 'large');//thickbox
            } else { //1.3
                $image_path = _PS_BASE_URL_.$link->getImageLink($o_product->link_rewrite, $product_id.'-'.$id_cover_image['id_image'], 'large');//thickbox
            }
