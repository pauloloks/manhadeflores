...
class ACorreiosServe {
    ...
    public function recuperaServicosCorreiosAtivos() 
    {
        ...
        $sql = "SELECT
                ...
                WHERE a_correios_servicos.id_shop = ".(int)$this->context->shop->id." AND
                      a_correios_servicos.ativo = 1";
    ...
    public function alteraCarrier($parm) 
    {
        ...
        if(!empty($files[$campoLogo]['name'])) {
            ...
            if (file_exists(Configuration::get('ACORREIOS_URI_LOGO_PS_TMP').$parm['idCarrier'].'_'.$this->context->shop->id.'.jpg')) {
                unlink(Configuration::get('ACORREIOS_URI_LOGO_PS_TMP').$parm['idCarrier'].'_'.$this->context->shop->id.'.jpg');
            }
    ...
    public function recuperaRastreio($codigo) 
    {
        ...
        $urlRastreio = str_replace('@', $codigo, Configuration::get('ACORREIOS_URL_RASTREIO_CORREIOS'));
