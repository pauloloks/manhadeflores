...
include_once('ACorreiosServe.php');
include_once('ACorreiosTransportadora.php');

class ACorreiosFrete 
{
    ...
    public function calculaFreteSimulador($origem, $dadosBasicos, $params) 
    {
        ...
        $fkclass = new ACorreiosServe();

        // Processa o frete
        $sql = "SELECT
                  a_correios_servicos.*,
                  "._DB_PREFIX_."carrier.id_reference,
                  a_correios_especificacoes.tabela_offline,
                  a_correios_especificacoes.servico,
                  a_correios_especificacoes.cod_servico,
                  a_correios_especificacoes.cod_administrativo,
                  a_correios_especificacoes.senha,
                  a_correios_especificacoes.valor_declarado_max,
                  a_correios_especificacoes.cubagem_max_isenta,
                  a_correios_especificacoes.cubagem_base_calculo,
                  a_correios_especificacoes.mao_propria_valor,
                  a_correios_especificacoes.aviso_recebimento_valor,
                  a_correios_especificacoes.valor_declarado_percentual,
                  a_correios_especificacoes.seguro_automatico_valor
                FROM a_correios_servicos
                  INNER JOIN "._DB_PREFIX_."carrier
                    ON a_correios_servicos.id_carrier = "._DB_PREFIX_."carrier.id_carrier
                  INNER JOIN a_correios_especificacoes
                    ON a_correios_servicos.id_especificacao = a_correios_especificacoes.id
                WHERE a_correios_servicos.ativo = 1 AND
                      a_correios_servicos.id_shop = ".(int)$this->context->shop->id;
        ...
        foreach ($servicos as $reg) 
        {
            ...
            if ($origem == 'produto') {
                ...
                $preco = $params['product']['price_amount'];
                $impostos = 0;
            ...
            // Processa embalagens
            switch(Configuration::get('ACORREIOS_EMBALAGEM')) {
            ...
            if (Configuration::get('ACORREIOS_FRETE_GRATIS_DEMAIS_TRANSP') != 'on' and $transpFreteGratisValor != $reg['id_carrier'] and $freteGratisValor or
                Configuration::get('ACORREIOS_FRETE_GRATIS_DEMAIS_TRANSP') != 'on' and $transpFreteGratisProdutos != $reg['id_carrier'] and $freteGratisProdutos) {
                continue;
            }
            ...
            $parm = array(
                ...
                'tempoPreparacao'           => Configuration::get('ACORREIOS_TEMPO_PREPARACAO'),
            ...
            if (Configuration::get('ACORREIOS_OFFLINE') == 'on') {
            ...
            $url_loja = $url_loja.'modules/acorreios/views/img/';

            $this->transportadoras[] = array(
                'url_logo'               => $url_loja.$reg['servico'].'.jpg',
                'nomeTransportadora'    => $reg['servico'],
                'prazoEntrega'          => $prazoEntrega,
                'mensagem'              => (Configuration::get('ACORREIOS_MSG_CORREIOS') == 'on' ? $retorno['msgCorreios'] : ''),
                'valorFrete'            => Tools::displayPrice($valorFrete),
            );
        }
    ...
    public function calculaFretePS($params, $idCarrier) {
        ...
        $sql = "SELECT
                ...
                FROM a_correios_servicos
                  ...
                  INNER JOIN a_correios_especificacoes
                    ON a_correios_servicos.id_especificacao = a_correios_especificacoes.id
                WHERE a_correios_servicos.ativo = 1 AND
                      a_correios_servicos.id_shop = ".(int)$this->context->shop->id. " AND
                      a_correios_servicos.id_carrier = ".(int)$idCarrier;
        ...
        if (Configuration::get('ACORREIOS_FRETE_GRATIS_DEMAIS_TRANSP') != 'on' and $transpFreteGratisValor != $servico['id_carrier'] and $freteGratisValor or
            Configuration::get('ACORREIOS_FRETE_GRATIS_DEMAIS_TRANSP') != 'on' and $transpFreteGratisProdutos != $servico['id_carrier'] and $freteGratisProdutos) {
            return false;
        }
        ...
        if (Configuration::get('ACORREIOS_OFFLINE') == 'on') {
            ...
            $retorno = $this->calculaValorOnline($parm, $params->id_carrier);
        }
        ...
        if($retorno['prazoEntrega'] > 1)
        {
            $delay = 'Entrega em até '.$prazoEntrega.' dias úteis';

        }
        else if ($retorno['prazoEntrega'] > 0)
        {
            $delay = 'Entrega em até 1 dia útil';
        }
    ...
    function getFreteGratis()
    {
       $contexto = Context::getContext();
       $id_shop = $contexto->shop->id;
       
       $fg = Db::getInstance()->ExecuteS('SELECT fr.*, es.cod_servico FROM a_correios_frete_gratis fr 
            left join a_correios_especificacoes es
            on es.id_shop = fr.id_shop
            AND es.id=fr.id_especificacao
            WHERE fr.id_shop='.$id_shop.' AND fr.regiao_cep > 0');

       $frete_gratis = array();

       foreach($fg as $frete)
       {
         $regs = explode('/', $frete['regiao_cep']);

         foreach($regs as $regiao)
         {
            if(!empty($regiao))
            {
                $regiao = str_replace(array(',','.', '-'), '', $regiao);
                list($inicio, $fim, $valor) = explode(':', $regiao);
                $frete_gratis[$frete['cod_servico']]['regioes'][] = array('inicio' => $inicio, 'fim' => $fim, 'valor' => $valor);
            }
         }
    ...
    private function calculaValorOnline($parm, $carrier=false) 
    {
        // Inicializa variaveis
        $prazoEntrega = 0;
        $msgCorreios = '';
        ...
        // Instancia ACorreiosTransportadora
        $correiosClass = new ACorreiosTransportadora();
        ...
        foreach ($parm['embalagens'] as $embalagem) {
            ...
            $total_pedido = $total_pedido + $embalagem['valorProdutos'];
            ...
            if (!empty($cache['status'])) 
            {
                ...
                $maoPropria = 'N';
                if (Configuration::get('ACORREIOS_MAO_PROPRIA') == 'on') {
                    $maoPropria = 'S';
                }

                $avisoRecebimento = 'N';
                if (Configuration::get('ACORREIOS_AVISO_RECEBIMENTO') == 'on') {
                    $avisoRecebimento = 'S';
                }

                $valorDeclarado = '0';
                if (Configuration::get('ACORREIOS_VALOR_DECLARADO') == 'on') {
                ...
                if ($correiosClass->calculaPrecoPrazo($parm['codServico'], $embalagem['valorProdutos'])) {
        ...
        #Veja se este pedido se enquadra para obter frete grátis

        $frete_gratis = $this->getFreteGratis();
                    
        if(!empty($frete_gratis[$parm['codServico']]))
        {
            foreach($frete_gratis[$parm['codServico']]['regioes'] as $fg)
            {

                if($fg['inicio'] <= $parm['cepDestino'] && $fg['fim'] >=$parm['cepDestino'] && $total_pedido >= $fg['valor'])
                {
                    $totalFrete = 'Grátis';
                }
            }
    ...
    private function calculaValorOffline($parm) {
        ...
        $minhaCidade = explode('/', Configuration::get('ACORREIOS_CEP_CIDADE'));
        ...
        if ($destino == 'cidade') {
            ...
            $sql = "SELECT
                    ...
                    WHERE a_correios_tabelas_offline.minha_cidade = 0 AND
                          a_correios_tabelas_offline.id_shop = ".(int)$this->context->shop->id." AND
                          a_correios_tabelas_offline.id_especificacao = ".(int)$parm['idEspecificacao']." AND
                          a_correios_cadastro_cep.estado = '".$parm['ufDestino']."'";
        }
    ...
    private function criaHash($idTranspAtual, $cepOrigem, $cepDestino, $embalagem) {
        ...
        $hash = $this->context->shop->id.':'.
                ...
                $cepOrigem.':'.
                $cepDestino.':'.
                Configuration::get('ACORREIOS_MAO_PROPRIA').':'.
                Configuration::get('ACORREIOS_VALOR_DECLARADO').':'.
                Configuration::get('ACORREIOS_AVISO_RECEBIMENTO').':'.
                $embalagem['altura'].':'.
                $embalagem['largura'].':'.
    ...
    private function gravaCache($retornoCorreios, $cepOrigem, $cepDestino, $embalagem) {
        ...
        foreach ($retornoCorreios as $retorno) {
            ...
            if (!$cache['status']) {
                ...
                $sql = 'INSERT INTO a_correios_cache (hash, valor_frete, prazo_entrega, msg_correios) VALUES 
                ("'.$dados['hash'].'", '.$dados['valor_frete'].', '.$dados['prazo_entrega'].', "'.$dados['msg_correios'].'")';
