...
class CartCore extends ObjectModel
{
    ...
    */
    ...
    public function getDeliveryOptionList(Country $default_country = null, $flush = false)
    {
        ...
        foreach ($delivery_option_list as $id_address => $delivery_option) {
            foreach ($delivery_option as $key => $value) {
                ...
                foreach ($value['carrier_list'] as $id_carrier => $data) {
                    ...
                    ################### CORREIOS ##############################
                    $external_module=$delivery_option_list[$id_address][$key]['carrier_list'][$id_carrier]['instance']->external_module_name;
                    $refaz_lista = false;
                    if($external_module=='acorreios')
                    {
                        //Pega um novo delay (prazo de entrega)
                        $refaz_lista = true;
                    }
                    ################### CORREIOS ##############################
        ...
        //Refaz lista com novo delay/prazo de entrega

        if($refaz_lista)
        {
            $correio = new Acorreios;
            $delivery_option_list = $correio->refazDeliveryOptionList($delivery_option_list);
        }
