...
class Posvegamenu extends Module {
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('Vertical Mega Menu Customer');
        $this->description = $this->l('block config');

        $this->confirmUninstall = $this->l('Are you sure you want to uninstall?');
        $this->admin_tpl_path = _PS_MODULE_DIR_ . $this->name . '/views/templates/admin/';
        if (!Configuration::get('POSVEGAMENU'))
            $this->warning = $this->l('No name provided');
    }

    public function install() {
        Configuration::updateGlobalValue('MOD_BLOCKPOSVENU_ITEMS', 'CAT3,CAT4,CAT5');
        Configuration::updateValue($this->name . '_show_homepage', 0);
        Configuration::updateValue($this->name . '_menu_depth', 2);
        Configuration::updateValue($this->name . '_merge_cate', 0);
        Configuration::updateValue($this->name . '_show_depth', 4);
        Configuration::updateValue($this->name . '_top_offset', 0);
        Configuration::updateValue($this->name . '_effect', 2);
		$this->installDb();
        return parent::install() &&
                $this->registerHook('displayHeader')
                &&   $this->registerHook('vegamenu');
    }
    ...
    public function getContent() {
        ...
        if (Tools::isSubmit('submitPosMegamenu')) {
			...
			{
				VegaTopLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
				$this->_html .= $this->displayConfirmation($this->l('The link has been added.'));
			}
		}
		else if (Tools::isSubmit('submitBlocktopmegaRemove'))
		{
			$id_linksvegatop = Tools::getValue('id_linksvegatop', 0);
			VegaTopLinks::remove($id_linksvegatop, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_BLOCKPOSVENU_ITEMS', str_replace(array('LNK'.$id_linksvegatop.',', 'LNK'.$id_linksvegatop), '', Configuration::get('MOD_BLOCKPOSVENU_ITEMS')));
    ...
    public function getStaticBlockContent($blockId = NULL, $task = NULL) {
        ...
        if (count($staticBlock) > 0) {
            ...
            $description = str_replace('/pos_genius/',__PS_BASE_URI__,$description);
            $html .= $description;
        }
        if ($task == 'item') {
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id) {
        ...
        $id = $category;
        // --- Static Block ---
        $blockId = sprintf('pt_ver_menu_idcat_%d', $id); // --- static block key
        $blockHtml = $this->getStaticBlockContent($blockId);
        /*check block right*/
        ...
        if ($drawPopup)
        {  
            $html[] = '<div id="ver_pt_menu' . $id . '" class="pt_menu pt_menu_block' . $active . '" >';
        }
        else
        {
            $html[] = '<div id="ver_pt_menu' . $id . '" class="pt_menu pt_menu_block noSub">';
        }
        // --- Top Menu Item ---
		$id_lang =  (int)Context::getContext()->language->id;
        ...
        if (count($files) > 0){
            foreach ($files as $file){
                if (preg_match('/^'.$cate->id.'-([0-9])?_thumb.jpg/i',$file) === 1) // set image categories 
                        $html[] = '<span class=" cate-thumb"><img class="img-cate" src="'.$this->context->link->getMediaLink(_THEME_CAT_DIR_.$file).'" alt= ""/></span>';
			}
		}
			 $names = strip_tags($cate->name);
			$name = str_replace('&nbsp;',' ', $names);
			$name = $this->l($name);
		$html[] = '<span>' . $name . '</span>';
	  //  $html[] = '<span><img src="'.$thumb.'" alt="" />' . $name . '</span>';
        $html[] = '</a>';
        $html[] = '</div>';
        ...
        if ($drawPopup)
        {
            ...
            $html[] = '<div id="ver_popup' . $id . '" class="popup">';
			//$html[] ='<div class="arrow-left-menu" id="arrow-left-menu3"></div>';
            // --- draw Sub Categories ---
    ...
    public function drawColumns($children, $id = NULL, $id_lang=NULL)
    {
        $html = '';
        // --- explode by columns ---
        $columns = Configuration::get($this->name . '_menu_depth');
        if ($columns < 1) $columns = 1;
        $chunks = $this->seperateColumns($children, $columns, $id_lang);
        ...
        foreach ($chunks as $key => $value)
        {
            ...
            $html.= $this->drawMenuItem($value, 1, $columChunk,$id_lang);
            $html.= '</div>';
        }
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/pos_genius/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
            $html[] = '<div id="ver_pt_menu' . $id . '" class="pt_menu">';
        } else {
            $html[] = '<div id="ver_pt_menu' . $id . '" class="pt_menu">';
        }
        // --- Top Menu Item ---
...
//        $html[] = '</a>';
    ...
    public function hookDisplayHeader() {
        ...
        $this->context->controller->addCSS($this->_path . 'css/posvegamenu.css');
	...
	public function hookvegamenu($params)
	{
		//$lang_id = (int) Configuration::get('PS_LANG_DEFAULT');
		$lang_id = (int)Context::getContext()->language->id;
        $category = new Category();
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = VegaTopLinks::get($link[0]['id_linksvegatop'], (int)$default_language, (int)Shop::getContextShopID());
						}
	...
	public function installDb(){
			...
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksvegatop` (
				...
				Db::getInstance()->execute('
				 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksvegatop_lang` (
				`id_linksvegatop` INT(11) UNSIGNED NOT NULL,
