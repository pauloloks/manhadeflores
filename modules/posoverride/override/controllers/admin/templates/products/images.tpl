...
<div id="product-images" class="panel product-tab">
	...
	<table class="table tableDnD" id="imageTable">
		<thead>
			<tr class="nodrag nodrop"> 
				...
				<th class="fixed-width-xs"><span class="title_box">{l s='Rotator Img'}</span></th>
				<th></th> <!-- action -->
			</tr>
		</thead>
		<tbody id="imageList">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var id_product = {/literal}{$id_product}{literal}
			//$('#link-Images').bind('click', function() {
					$.ajax({
						type: 'POST',
						url:'../modules/posrotatorimg/ajax.php',
						data: 'id_product='+id_product+'&action=2'+'&shop_id='+current_shop_id,
						dataType: 'json',
						success: function(json) { 
							console.log('succes');
							data = JSON.parse(json);
							console.log(data.id_image);
							var td = '#'+data.id_image;
							var rotator = $(td).find('input[name="rotator"]').attr('checked','checked');
						},
			...
			$('input[name="rotator"]').live('click',function(){
				var id_product = {/literal}{$id_product}{literal}
				...
				$.ajax({
						type: 'POST',
						url:'../modules/posrotatorimg/ajax.php',
						data: 'img_id='+id+'&id_product='+id_product+'&action=1'+'&shop_id='+current_shop_id,
						dataType: 'json',
						success: function(json) {
