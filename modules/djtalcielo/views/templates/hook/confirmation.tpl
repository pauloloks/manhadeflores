...
{if (isset($status) == true) && ($status == 'ok')}
    ...
    <p>
        <br />- {l s='Amount' mod='djtalcielo'} : <span class='price'><strong>{$total|escape:'htmlall':'UTF-8'}</strong></span>
...
{else}
    <h3>{l s='Your order on %s has not been accepted.' sprintf=$shop_name mod='djtalcielo'}</h3>
    <p>
        <br />- {l s='Reference' mod='djtalcielo'} <span class='reference'> <strong>{$reference|escape:'html':'UTF-8'}</strong></span>
