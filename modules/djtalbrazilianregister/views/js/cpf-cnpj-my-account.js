...
$(document).on('ready', function(){
	...
	$('#fiscalInformationForm button[name=submitFiscalInformation]').on('click', function(event){
		var ret = true;
		if($('#br_document_cpf').length > 0) {
			$('#br_document_cpf').parent().removeClass('form-error');
			$('#br_document_cpf').parent().removeClass('form-ok');
			if(validate_isCPF($('#br_document_cpf').val())){
				$('#br_document_cpf').parent().addClass('form-ok');
			}else{
				$('#br_document_cpf').parent().addClass('form-error');
				event.stopPropagation();
				ret = false;
			}
		}
		if($('#br_document_cnpj').length > 0) {
			$('#br_document_cnpj').parent().removeClass('form-error');
			$('#br_document_cnpj').parent().removeClass('form-ok');
			if(validate_isCNPJ($('#br_document_cnpj').val())){
				$('#br_document_cnpj').parent().addClass('form-ok');
			}else{
				$('#br_document_cnpj').parent().addClass('form-error');
				event.stopPropagation();
				ret = false;
			}
		}
