...
class Staticfooter extends ObjectModel
{
    ...
    public  function getStaticfooterLists($id_shop = NULL, $hook_position= 'top') { 
		...
		');
                ...
                if(count($newObject>0)) {
                    foreach($object as $key=>$ob) {
					   ...
					   $is_active = $ob['insert_module'];
                       $blockModule = $this->getModuleAssign($nameModule, $hookModule,$is_active);
       ...
       public  function getModuleAssign( $module_name = '', $name_hook = '', $is_active = 0 ){
		
		if(!$module_name || $module_name == 'Chose Module' || !$name_hook || $is_active ==0)  return ;
